...
public class Swerve extends SubsystemBase {
    ...
    public void swerveStop() {
        this.drive(new Translation2d(0, 0), 0, true);
    }
    ...
    public ChassisSpeeds getTargetSpeeds(double xInput, double yInput, Rotation2d angle, double maxSpeed) {
        ...
        public Pose2d getEarlyPoseMoving(double dt) {
        Pose2d actualPose = getPose();
        Translation2d earlyTranslation = new Translation2d(
                actualPose.getX() + getRobotVelocity().vxMetersPerSecond * dt,
                actualPose.getY() + getRobotVelocity().vyMetersPerSecond * dt);
        Pose2d earlyMoving = new Pose2d(earlyTranslation, actualPose.getRotation());
        return earlyMoving;
    }
    
  public Pose2d getEarlyPoseMovingWithOmega(double dt) {
    Pose2d actualPose = getPose();
    Translation2d earlyTranslation = new Translation2d(
        actualPose.getX() + getRobotVelocity().vxMetersPerSecond * dt,
        actualPose.getY() + getRobotVelocity().vyMetersPerSecond * dt);
